 ...
 * @subpackage database
 * 
 * Class to MsSQL database functionality.
 * 
**/


include_once(dirname(__FILE__).'/../../PHP-Tinned-Core/classes/main.class.php');
include_once(dirname(__FILE__).'/../../PHP-Tinned-Core/classes/debug_logging.interface.php');


/**
 * MsSQL SERVER CONNECTION CLASS
 * 
 * The MsSQL class is an abstraction class to abstract the differences 
 * between different sql type away. This class is abstracting away the MsSQL 
...
class MsSQL extends Main
{
    ...
    **/
     ...
     * follows:
     * mssql://username:password@servername:port/database
     * mssqlp://username:password@servername:port/database
     * 
     * @param $connection_string - The sql connection string
    **/
    public function set_connection_string($connection_string)
    {
        ...
        //     mssql://username:password@servername/database
        //     mssql://username:password@servername:port/database
        // parse connection string of format (persistant)
        //     mssqlp://username:password@servername/database
        //     mssqlp://username:password@servername:port/database
    ...
    **/
    public function connection(&$errno, &$errstr)
    {
        ...
        if(empty($this->sql_handle) === TRUE)
        {
            ...
            {
                $this->sql_handle = @mssql_pconnect($this->db_host,$this->db_user,$this->db_pass);
            }
            else
            {
                $this->sql_handle = @mssql_connect($this->db_host,$this->db_user,$this->db_pass);
            }
            ...
            {
                $errno = 101;
                $errstr = "Connect to mssql Server Error: ".mssql_get_last_message();
                // not required
                //
                parent::error($errstr);
                return FALSE;
            ...
            // Select Database
            $this->db_link = @mssql_select_db($this->db_name, $this->sql_handle);
            if(!$this->db_link)
            {
                $errno = 102;
                
                
                $errstr = "Connect to DB Error. DBName = $this->db_name ".mssql_get_last_message();
                parent::error($errstr);
                return FALSE;
            }
            ...
            {
                $errno = 902;
                $errstr = "Connect to mssql DB Sucess. DBName = $this->db_name";
            }
    ...
    public function query($string, &$errno, &$errstr, &$last_insert_id = FALSE)
    {
        ...
        if(empty($this->sql_handle))
        {
            if($this->connection($errno, $errstr) === FALSE)
            {
                // Not required
                //
                parent::error("Query ... Error = ".mssql_get_last_message());
                return FALSE;
            }
        }
        ...
        parent::performance("", "start");
        $this->query_result = @mssql_query($string, $this->sql_handle);
        if($last_insert_id !== FALSE)
        {
            // read the last insert ID
            $last_insert_id = $this->_last_insert_id($this->sql_handle);
    ...
    **/
    function objects($string = "",$qid = "", &$errno, &$errstr)
    {
        ...
        if($string != "")
        {
            ...
            $objects = @mssql_fetch_object($this->query_result);
            //
            //
            //
        }
    ...
    function row_array_assoc($string = "",$qid = "", &$errno, &$errstr)
    {
        ...
        {
            $this->query($string, $errno, $errstr);
            $objects = @mssql_fetch_assoc($this->query_result);
            //
            //
            //
        }
    ...
    **/
    private function _check_prerequisits()
    {
        ...
        $function_list = array('mssql_connect', 'mssql_pconnect', 'mssql_get_last_message', 'mssql_select_db', 
            'mssql_close', 'mssql_query', 'mssql_num_rows', 'mssql_fetch_object', 'mssql_fetch_assoc');
        //
    ...
    **/
    ...
    private function _num_rows($query_result, $query = NULL)
    {
        ...
        // MsSQL returns a BOOL FALSE when no rows were found
        if(is_resource($query_result) == FALSE)
        {
            return 0;
        }
        ...
        switch($sql_command)
        {
            ...
            case "SHOW":
                $result = "num";
                parent::debug2("Detected command '$sql_command' is recognized for mssql_".$result."_rows().");
            ...
            case "REPLACE":
            case "DELETE":
            // case "EXEC": commented out because rows_affected appears to clear the result set
                $result = "affected";
                parent::debug2("Detected command '$sql_command' is recognized for mssql_".$result."_rows().");
                $this->query_type = $result;
                return @mssql_rows_affected($this->sql_handle);
                break;
            default:
                $result = "num";
                parent::debug2("Detected command '$sql_command' is recognized for mssql_".$result."_rows() - DEFAULT.");
                $this->query_type = $result;
                // return result of function if result is a resource, 0 otherwise. To avoid PHP_ERR on boolean query_result
                if(is_resource($query_result) === TRUE)
                {
    ...
    **/
    ...
    {
        // send the query to get the last insert id
        $query_result = @mssql_query("SELECT SCOPE_IDENTITY() AS last_insert_id;", $this->sql_handle);
        $objects = @mssql_fetch_assoc($query_result);
        
        // return the last insert id
        return $objects['last_insert_id'];
    }
