 ...
 * @subpackage database
 *
 * Class to SQLite2 database functionality.
 * 
**/
...
/**
 * SQLite2 CONNECTION CLASS
 * 
 * The SQLite2 class is an abstraction class to abstract the differences 
 * between different sql type away. This class is abstracting away the sqlite2 
...
class SQLite_2 extends Main
{
    ...
    **/
    public function connection(&$errno, &$errstr)
    {
        ...
        if(empty($this->sql_handle) === TRUE)
        {
            ...
            {
                $this->sql_handle = @sqlite_popen($this->db_host, 0666, $this->sql_error);
            }
            else
            {
                $this->sql_handle = @sqlite_open($this->db_host, 0666, $this->sql_error);
            }
            ...
            {
                $errno = 101;
                $errstr = "Connect to sqlite2 Error";
                // not required
                //
                parent::error("Connect ... Connection to Server FAILED - Error = ".$this->sql_error);
                return FALSE;
            }
    ...
    public function query($string, &$errno, &$errstr, &$last_insert_id = FALSE)
    {
        ...
        if(empty($this->sql_handle))
        {
            if($this->connection($errno, $errstr) === FALSE)
            {
                // Not required
                //
                parent::error("Query ... Error = ".$this->sql_error);
                return FALSE;
            }
        }
        ...
        parent::performance("", "start");
        $this->query_result = @sqlite_query($this->sql_handle, $string, SQLITE_ASSOC, $this->sql_error);
        if($last_insert_id !== FALSE)
        {
            // read the last insert ID
            $last_insert_id = sqlite_last_insert_rowid($this->sql_handle);
    ...
    **/
    function nums($string = "",$qid = "", &$errno, &$errstr)
    {
        ...
        if($string != "")
        {
            ...
            $this->query($string);
            $this->total = @sqlite_num_rows($this->query_result);
            //
            //
            //
        }
        ...
        {
            // retrive the number of results
            $this->total = @sqlite_num_rows($qid);
            //
            //
            //
        }
    ...
    **/
    function objects($string = "",$qid = "", &$errno, &$errstr)
    {
        ...
        if($string != "")
        {
            ...
            $objects = @sqlite_fetch_object($this->query_result);
            //
            //
            //
        }
        elseif($qid != "")
        {
            $objects = @sqlite_fetch_object($qid);
            //
            //
            //
        }
    ...
    function row_array_assoc($string = "",$qid = "", &$errno, &$errstr)
    {
        ...
        {
            $this->query($string, $errno, $errstr);
            $objects = @sqlite_fetch_array($this->query_result, SQLITE_ASSOC);
            //
            //
            //
        }
        elseif($qid != "")
        {
            $objects = @sqlite_fetch_array($qid, SQLITE_ASSOC);
            //
            //
            //
        }
    ...
    **/
    private function _check_prerequisits()
    {
        ...
        $function_list = array('sqlite_open', 'sqlite_popen', 
            'sqlite_close', 'sqlite_query', 'sqlite_num_rows', 'sqlite_fetch_object', 'sqlite_fetch_array', 
